...
@interface STSScrollView : UIScrollView
@end
...
@property (nonatomic, strong) STSScrollView *scrollView;
...
- (id)init
{
	...
	self.type = STSSegmentedControlTypeText;
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = STSSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = STSSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[STSScrollView alloc] init];
    ...
    self.selectionStyle = STSSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = STSSegmentedControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = STSSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(STSSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == STSSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(STSSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force STSSegmentedControlSegmentWidthStyleFixed when type is STSSegmentedControlTypeImages.
    if (self.type == STSSegmentedControlTypeImages) {
        _segmentWidthStyle = STSSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(STSSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == STSSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == STSSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != STSSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleFixed) {
				...
				//rectDiv = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight, self.verticalDividerWidth, self.frame.size.height - self.selectionIndicatorHeight);
                fullRect = CGRectMake(self.segmentWidth * idx, 0, self.segmentWidth, oldRect.size.height);
            } else if (self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == STSSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == STSSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
            ...
            if (self.isVerticalDividerEnabled && idx>0) {
				...
				//verticalDividerLayer.frame = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight, self.verticalDividerWidth, self.frame.size.height-self.selectionIndicatorHeight);
                verticalDividerLayer.backgroundColor = self.verticalDividerColor.CGColor;
    ...
    } else if (self.type == STSSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != STSSegmentedControlNoSegment) {
        if (self.selectionStyle == STSSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == STSSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & STSSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == STSSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == STSSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == STSSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == STSSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != STSSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == STSSegmentedControlTypeText && self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == STSSegmentedControlTypeText && self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == STSSegmentedControlTypeTextImages && self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == STSSegmentedControlTypeTextImages && self.segmentWidthStyle == STSSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == STSSegmentedControlTypeTextImages || self.type == STSSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == STSSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == STSSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == STSSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
...
{
	CGRect r = [@"X" boundingRectWithSize:CGSizeMake(200, 0)
								  options:NSStringDrawingUsesLineFragmentOrigin
							   attributes:[self resultingTitleTextAttributes]
								  context:nil];
	
	STSDisplay * dpy = [STSDisplay instance];
	float m = [dpy centimetersToScreenUnits:0.6];

	if(_type == STSSegmentedControlTypeTextImages)
		m *= 1.5;

	float h = r.size.height > m ? r.size.height : m;
	
	return CGSizeMake(100.0,r.size.height);
}

- (CGSize)sizeThatFits:(CGSize)size
{
	CGSize s = [self intrinsicContentSize];
	return CGSizeMake(size.width,s.height > size.height ? size.height : s.height);
}
